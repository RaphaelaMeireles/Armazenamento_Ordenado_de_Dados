...
TipoCelulaFila *Frente, *Tras; //Variáveis utilizadas para manter o controle do estado atual da Fila
                           //Como a Fila é implementada em memória dinâmica,
                           //Frente e Tras devem ser ponteiros para as células da Fila


int ContadorFila;  //Variável utilizada para armazenar a quantidade de elementos na Fila
...
void Fila_Destrutor()
{
   ...
   TipoCelulaFila *Temp;  //Ponteiro temporário utilizado para desalocar a memória
...
int Fila_Tamanho()
{
    return ContadorFila;  //É retornado o contador de elementos, manipulado nos métodos de Enfileirar e Desenfileirar
}
...
bool Fila_Enfileirar(TipoDadoFila Elemento)
{
   ...
   TipoCelulaFila *Nova;  //Ponteiro temporário utilizado para apontar para a nova célula a ser criada
...
bool Fila_Desenfileirar(TipoDadoFila &Elemento)
{
     ...
     if(!Fila_Vazia()){  //Se a Fila não estiver vazia, podemos remover algum elemento
        ...
        TipoCelulaFila *Temp = Frente;//Cria um ponteiro temporário (Temp) e faz apontar para o mesmo endereço de Frente
        ...
        ContadorFila--;  //"Contador" é decrementado para indicar que um elemento foi removido da Fila (desenfileirado)
